...
import { mkdir, rm, readFile, writeFile, stat, readdir, copyFile } from 'node:fs/promises';
 ...
 */
function isCliBuildAvailable(): boolean {
  ...
  describe('Error Handling Tests (TC-CLI-010)', () => {
    it('should error on non-existent file', { timeout: 30000 }, async () => {
      ...
      expect(result.stderr + result.stdout).toMatch(
        /error|not found|ENOENT|no such file/i
      );
    });
    ...
    it('should error on invalid format name', { timeout: 30000 }, async () => {
      ...
      expect(result.stderr + result.stdout).toMatch(
        /unknown format|invalid|not supported/i
      );
    });
