...
import { runCompress, CompressOptions } from './batch/index.js';
...
program
  ...
  .option(
    '-f, --format <format>',
    `Output format (${listEncoders().join('|')})`,
    'mozjpeg',
  )
  ...
  .action(async (input: string, options: Record<string, unknown>) => {
    try {
      const compressOptions: CompressOptions = {
        ...
        resize: options.resize as string | undefined,
        parallel: (options.parallel as string) || '',
        continueOnError: options.continueOnError as boolean,
        replace: options.replace as boolean,
        fast: options.fast as boolean, // --no-fast sets this to false
      };
...
program
  ...
  .action(() => {
    ...
    for (const encoder of encoders) {
      ...
      console.log(
        `  ${chalk.cyan(name)}  ${chalk.gray(label)}  ${chalk.yellow(ext)}`,
      );
    }
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      ...
      const allowedDirs = options.allowDir
        ? (options.allowDir as string).split(',').map((d) => d.trim())
        : [];
