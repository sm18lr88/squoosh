...
const FIXTURES = {
  ...
  demoJpg: join(
    PROJECT_ROOT,
    'src/shared/prerendered-app/Intro/imgs/demos/demo-large-photo.jpg',
  ),
};
 ...
 */
async function createBatchTestImages(
  dir: string,
  count: number,
  size: number = 100,
): Promise<string[]> {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const filePath = join(
      dir,
      `test_image_${i.toString().padStart(3, '0')}.png`,
    );
...
/**
 * Calculates average time from encoder results
 */
function calculateAverageTime(results: Array<{ time: number }>): number {
  return results.reduce((sum, r) => sum + r.time, 0) / results.length;
}

/**
 * Calculates average size from encoder results
 */
function calculateAverageSize(results: Array<{ size: number }>): number {
  return results.reduce((sum, r) => sum + r.size, 0) / results.length;
}

/**
 * Logs encoder statistics for a specific encoder
 */
function logEncoderStats(
  encoderName: string,
  allResults: Array<{ encoder: string; size: number; time: number }>,
): void {
  const encoderResults = allResults.filter((r) => r.encoder === encoderName);
  const avgTime = calculateAverageTime(encoderResults);
  const avgSize = calculateAverageSize(encoderResults);
  console.log(
    `    ${encoderName}: avg ${avgTime.toFixed(0)}ms, ${(avgSize / 1024).toFixed(0)} KB`,
  );
}
...
describePerf('Performance Benchmarks', () => {
  ...
  describe('TC-PERF-001: Large File Handling', () => {
    it('should compress large PNG file without timeout', async () => {
      ...
      console.log(
        `    Input file size: ${(inputStat.size / 1024 / 1024).toFixed(2)} MB`,
      );

      // Decode the image
      const { result: imageData, elapsed: decodeTime } = await measureTime(() =>
      ...
      const encodeOptions = {
        ...(encoder.defaultOptions as Record<string, unknown>),
        quality: 75,
      };
      const { result: encoded, elapsed: encodeTime } = await measureTime(() =>
        encoder.encode(imageData, encodeOptions),
      );

      console.log(`    Encode time: ${encodeTime.toFixed(0)}ms`);
      console.log(
        `    Output size: ${(encoded.byteLength / 1024).toFixed(0)} KB`,
      );
    ...
    it('should handle very large synthetic image (5000x5000)', async () => {
      ...
      console.log(
        `    Created ${width}x${height} image (${((width * height * 4) / 1024 / 1024).toFixed(1)} MB raw)`,
      );
      ...
      const largeEncodeOptions = {
        ...(encoder.defaultOptions as Record<string, unknown>),
        quality: 75,
      };
      const { result: encoded, elapsed } = await measureTime(() =>
        encoder.encode(imageData, largeEncodeOptions),
      );

      console.log(`    WebP encode time: ${elapsed.toFixed(0)}ms`);
      console.log(
        `    Output size: ${(encoded.byteLength / 1024 / 1024).toFixed(2)} MB`,
      );
  ...
  describe('TC-PERF-003: Batch Performance', () => {
    ...
    it('should create and process 25+ test images', async () => {
      ...
      const { result: imagePaths, elapsed: createTime } = await measureTime(
        () => createBatchTestImages(batchDir, BATCH_SIZE, 200),
      );
      ...
      const results: { path: string; inputSize: number; outputSize: number }[] =
        [];
      ...
      console.log(
        `    Average per file: ${(batchTime / BATCH_SIZE).toFixed(0)}ms`,
      );
      ...
      const compressionRatio = ((1 - totalOutput / totalInput) * 100).toFixed(
        1,
      );
    ...
    it('should measure throughput with parallel simulation', async () => {
      ...
      const imagePaths = await createBatchTestImages(
        batchDir,
        PARALLEL_BATCH,
        150,
      );
      ...
      console.log(
        `    Sequential time (${PARALLEL_BATCH} files): ${sequentialTime.toFixed(0)}ms`,
      );

      // Parallel processing (limited concurrency)
      const CONCURRENCY = 4;

      // Extract processing task to reduce nesting
      const processImageFile = async (filePath: string): Promise<void> => {
        const imageData = await decodeFile(filePath);
        await encoder.encode(imageData, seqEncodeOptions);
      };
      ...
      const { elapsed: parallelTime } = await measureTime(async () => {
        ...
        while (queue.length > 0 || active.length > 0) {
          ...
          while (active.length < CONCURRENCY && queue.length > 0) {
            ...
            active.push(processImageFile(filePath));
          }
      ...
      console.log(
        `    Parallel time (concurrency=${CONCURRENCY}): ${parallelTime.toFixed(0)}ms`,
      );
      console.log(
  ...
  describe('Concurrent Operations', () => {
    it('should handle concurrent encodings without corruption', async () => {
      ...
      console.log(
        '    Results:',
        results.map((r) => `${r.id}=${r.size}B`).join(', '),
      );
    ...
    it('should handle rapid sequential encodings', async () => {
      ...
      console.log(
        `    ${iterations} rapid encodings in ${elapsed.toFixed(0)}ms`,
      );
      console.log(
        `    Average: ${(elapsed / iterations).toFixed(0)}ms per encode`,
      );
  ...
  describe('Memory Stress Test', () => {
    it('should process multiple large images sequentially without crashing', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        console.log(
          `    Iteration ${i + 1}: ${currentMemory.toFixed(1)} MB (output: ${(encoded.byteLength / 1024).toFixed(0)} KB)`,
        );
      }
    ...
    it('should handle alternating between different encoders', async () => {
      ...
      const results: { encoder: string; size: number; time: number }[] = [];
      ...
      for (let round = 0; round < 3; round++) {
        for (const name of encoderNames) {
          const encoder = getEncoder(name)!;
          const { result: encoded, elapsed } = await measureTime(() =>
            encoder.encode(imageData, encoder.defaultOptions),
          );
      ...
      // Log results using top-level helper function
      encoderNames.forEach((name) => {
        logEncoderStats(name, results);
      });
  ...
  describe('Edge Case Performance', () => {
    it('should handle 1x1 pixel image quickly', async () => {
      ...
      for (const name of encoderNames) {
        ...
        console.log(
          `    ${name}: ${elapsed.toFixed(1)}ms, ${encoded.byteLength} bytes`,
        );
    ...
    it('should handle very wide image (10000x10)', async () => {
      ...
      console.log(
        `    Image: 10000x10 (${((10000 * 10 * 4) / 1024).toFixed(0)} KB raw)`,
      );

      const encoder = getEncoder('webp')!;
      await encoder.warmup();

      const wideEncodeOptions = {
        ...(encoder.defaultOptions as Record<string, unknown>),
        quality: 80,
      };
      const { result: encoded, elapsed } = await measureTime(() =>
        encoder.encode(imageData, wideEncodeOptions),
      );

      console.log(`    Encode time: ${elapsed.toFixed(0)}ms`);
      console.log(
        `    Output size: ${(encoded.byteLength / 1024).toFixed(1)} KB`,
      );
    ...
    it('should handle very tall image (10x10000)', async () => {
      ...
      console.log(
        `    Image: 10x10000 (${((10 * 10000 * 4) / 1024).toFixed(0)} KB raw)`,
      );

      const encoder = getEncoder('webp')!;
      await encoder.warmup();

      const tallEncodeOptions = {
        ...(encoder.defaultOptions as Record<string, unknown>),
        quality: 80,
      };
      const { result: encoded, elapsed } = await measureTime(() =>
        encoder.encode(imageData, tallEncodeOptions),
      );
    ...
    it('should handle square power-of-two dimensions efficiently', async () => {
      ...
      const results: { size: number; time: number; output: number }[] = [];

      const powerOfTwoEncodeOptions = {
        ...(encoder.defaultOptions as Record<string, unknown>),
        quality: 80,
      };
      for (const size of sizes) {
        const imageData = createTestImageData(size, size);
        const { result: encoded, elapsed } = await measureTime(() =>
          encoder.encode(imageData, powerOfTwoEncodeOptions),
        );
      ...
      results.forEach((r) => {
        console.log(
          `    ${r.size.toString().padStart(9)} | ${r.time.toFixed(0).padStart(9)} | ${(r.output / 1024).toFixed(1).padStart(11)}`,
        );
      });
  ...
  describe('Encoder Comparison', () => {
    it('should benchmark all encoders with the same image', async () => {
      ...
      const results: { name: string; time: number; size: number }[] = [];
      ...
      results.forEach((r) => {
        ...
        console.log(
          `    ${r.name.padEnd(8)} | ${r.time.toFixed(0).padStart(9)} | ${(r.size / 1024).toFixed(1).padStart(9)} | ${compression.padStart(10)}%`,
        );
      });
...
describe('Performance Tests Info', () => {
  it('should provide instructions for running performance tests', () => {
    if (RUN_PERF_TESTS) {
      ...
      console.log(
        '    RUN_PERF_TESTS=true npx vitest run src/cli/performance.test.ts\n',
      );
    }
