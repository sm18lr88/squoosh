...
// Use npx to run tsc to handle cross-platform differences
const tscCommand = 'npx';
const tscBaseArgs = ['tsc'];
  ...
  async function tsBuild(rollupContext) {
    ...
    tsBuildDone = Promise.resolve().then(async () => {
      await new Promise((resolve) => {
        const proc = spawn(tscCommand, [...tscBaseArgs, ...args], {
          stdio: 'inherit',
          shell: true,
        });
      ...
      if (watch) {
        tsBuildDone.then(() => {
          spawn(
            tscCommand,
            [...tscBaseArgs, ...args, '--watch', '--preserveWatchOutput'],
            {
              stdio: 'inherit',
              shell: true,
            },
