...
describe('ImageData polyfill', () => {
    describe('constructor with width and height', () => {
        ...
        it('should throw RangeError for zero width', () => {
            ...
            expect(() => new PolyfillImageData(0, 10)).toThrow("Failed to construct 'ImageData': The width is zero or not a number.");
        });
        it('should throw RangeError for zero height', () => {
            expect(() => new PolyfillImageData(10, 0)).toThrow(RangeError);
            expect(() => new PolyfillImageData(10, 0)).toThrow("Failed to construct 'ImageData': The height is zero or not a number.");
        });
    ...
    describe('constructor with data', () => {
        ...
        it('should throw TypeError for non-Uint8ClampedArray data', () => {
            ...
            expect(() => new PolyfillImageData(data, 2, 5)).toThrow(TypeError);
            expect(() => new PolyfillImageData(data, 2, 5)).toThrow("Failed to construct 'ImageData': parameter 1 is not of type 'Uint8ClampedArray'.");
        });
        it('should throw RangeError for data length mismatch', () => {
            const data = new Uint8ClampedArray(40); // 2x5 pixels
            expect(() => new PolyfillImageData(data, 3, 5)).toThrow(RangeError);
            expect(() => new PolyfillImageData(data, 3, 5)).toThrow("Failed to construct 'ImageData': The input data byte length is not a multiple of (4 * width * height).");
        });
