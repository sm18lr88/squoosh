...
import { isSharpAvailable, isFormatSupportedBySharp, processWithSharp, } from '../codecs/sharp-backend.js';
import { isNativeAvailable, processWithNative, getOptimalConcurrency, } from '../codecs/native-backend.js';
/**
 * Runs the compress command on the specified input
 * @param input - Path to file or directory to process
 * @param options - Compression options
 */
export async function runCompress(input, options) {
    // 1. Validate format
    ...
    const manualParallel = options.parallel ? parseInt(options.parallel, 10) : 0;
 ...
 */
async function processFiles(files, encoder, quality, outputOptions, processingOptions) {
    ...
    await new Promise((resolve, reject) => {
        const processNext = async () => {
            ...
            activeCount++;
            try {
                const result = await processFile(file, encoder, outputOptions, processingOptions);
 ...
 */
async function processFile(inputPath, encoder, outputOptions, processingOptions) {
    ...
    if (useSharp) {
        ...
        const encodeOptions = { ...encoder.defaultOptions, quality };
        encoded = await encoder.encode(imageData, encodeOptions);
    }
    // 3. Write output
    const outputPath = await writeOutputFile(inputPath, encoded, encoder.extension, outputOptions);
