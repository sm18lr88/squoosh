...
import { promptInputType, promptFormat, promptQuality, promptOutputDir, confirmSettings, } from './prompts.js';
import { decodeFile, getSupportedExtensions } from '../codecs/decoders/index.js';
import { createProgressReporter, formatBytes, formatSizeDiff, } from '../utils/progress.js';
        ...
        for (const filePath of input.paths) {
            ...
            try {
                ...
                if (!supportedExtensions.includes(ext)) {
                    console.log(chalk.yellow(`  Warning: ${filePath} has unsupported format ${ext}, skipping.`));
                    continue;
                }
 ...
 */
async function processFiles(files, format, quality, outputDir) {
    const results = [];
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            // Write output file
            const outputPath = await writeOutputFile(filePath, encoded, format.encoder.extension, outputOptions);
    ...
    if (failCount === 0) {
        ...
        progress.succeed(`Processed ${successCount} file(s), ${failCount} failed`);
    }
    // Show summary
...
function printSummary(results) {
    ...
    if (successful.length > 0) {
        ...
        for (const result of successful) {
            ...
            console.log(`  ${chalk.gray(inputName)} -> ${chalk.cyan(outputName)}  ${diffColor(sizeDiff)}`);
        }
        // Total savings
        if (successful.length > 1) {
            ...
            console.log(chalk.bold('\n  Total:'));
            console.log(`  ${formatBytes(totalInputSize)} -> ${formatBytes(totalOutputSize)}  ${totalDiffColor(totalDiff)}`);
        }
    }
    if (failed.length > 0) {
