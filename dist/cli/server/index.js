...
import { isSharpAvailable, processWithSharp, encodeWithSharp, } from '../codecs/sharp-backend.js';
    ...
    const server = createServer(async (req, res) => {
        ...
        try {
            ...
            if (pathname === '/process' && req.method === 'POST') {
                const { fields, file, fileName } = await parseMultipart(req);
                if (!file) {
                    sendError(res, 400, 'No file provided');
                    return;
                }
                const format = fields.format || 'webp';
                const quality = parseInt(fields.quality || '75', 10);
                // Decode the image
